...
funcionE.empleadosAccessAll = (acc_andon, sign, callback) => {
    ...
    dbE.query(`SELECT acc_id FROM del_accesos WHERE acc_calidad ${sign}${acc_andon}`, function (err, result, fields) {
        if (err) {
            callback(err, null);
        } else {
...
funcionE.empleadosAccesos = (callback) => {
    ...
    dbE.query(`SELECT acc_id, acc_calidad FROM del_accesos WHERE acc_calidad>0`, function (err, result, fields) {
        if (err) {
            callback(err, null);
        } else {
...
funcionE.empleadosDeleteAcceso = (acc_id, callback) => {
    ...
    dbE.query(`UPDATE del_accesos SET acc_calidad=0 WHERE acc_id=${acc_id}`, function (err, result, fields) {
            if (err) {
...
funcionE.empleadosInsertAcceso = (acc_id, acc_calidad, callback) => {

    dbE.query(`INSERT INTO del_accesos (acc_id, acc_calidad)
    VALUES('${acc_id}',${acc_calidad})
    ON DUPLICATE KEY UPDATE acc_calidad= VALUES(acc_calidad)`, function (err, result, fields) {
            if (err) {
